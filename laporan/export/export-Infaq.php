...
// Ambil data database (ditulis satu baris agar rapi)
$data = mysqli_query($connect, "SELECT * FROM infaq ORDER BY tanggal ASC, id_infaq ASC");
...
$jumlahDonatur = $totalDonatur['total'] ?? 0;
$jumlahTransaksi = $totalTransaksi['total'] ?? 0;
$totalNominal = $totalInfaq['total'] ?? 0;
...
<table class="summary">
    <tr>
        <td>
            <span class="summary-label">JUMLAH DONATUR</span>
            <span class="summary-value">' . $jumlahDonatur . ' Orang</span>
        </td>
        <td>
            <span class="summary-label">TOTAL INFAQ</span>
...
if (mysqli_num_rows($data) > 0) {
    ...
    $html .= '
        <tr>
            <td colspan="4" class="empty">Belum ada data infaq.</td>
        </tr>';
}
...
$dompdf->render();
$dompdf->stream('Laporan-Infaq.pdf', ['Attachment' => false]);